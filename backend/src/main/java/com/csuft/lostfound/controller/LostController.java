...
public class LostController {
    ...
    public LostController(ItemService itemService) {
    ...
    public ApiResponse<PageResult<ItemView>> list(
        ...
        return ApiResponse.success(itemService.listItems("lost", keyword, category, location, tag, status, reviewStatus, pageNum, pageSize));
    }

    @PostMapping("/publish")
    public ApiResponse<Long> publish(@Valid @RequestBody PublishItemRequest request) {
        return ApiResponse.success(itemService.publish("lost", request));
    }

    @PutMapping("/update/{id}")
    public ApiResponse<Boolean> update(@PathVariable Long id, @RequestBody UpdateItemRequest request) {
        return ApiResponse.success(itemService.update("lost", id, request));
    }

    @DeleteMapping("/delete/{id}")
    public ApiResponse<Boolean> delete(@PathVariable Long id) {
        return ApiResponse.success(itemService.delete("lost", id));
    }
}
